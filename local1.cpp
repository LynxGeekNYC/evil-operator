...
int main()
{
	...
	int newline[11];
	int line=2, count, randForFirstFour;
	...
	int firstFour[5][4] = { {1,7,1,8},{1,6,4,6},{1,2,1,2},{1,7,1,8},{1,9,2,9} };
	for (int i = 0; i < 11; i++)
	{
		
		if (i == 0)
		{
			randForFirstFour = rand() % 4;
			for (int j = 0; j < 4; j++)
			{
				GeneratedNum[i++] = firstFour[randForFirstFour][j];
			}
			i--;
		}
		else if (i == 4)
